...
import { Friend, Colleague, SecureFriendContact, FriendPartial, EventPass,IntersectionList} from "./myTypes";
...
function secureFindFriends(friends: Friend[], criteria: (f: Friend) => boolean): SecureFriendContact[] {
  ...
  return matches.map((f) => {
    const secure: SecureFriendContact = {name: f.name, phone: f.phone};
    return secure;
  });
}
...
function intersection(friends: Friend[],colleagues: Colleague[]): IntersectionList[] {
  let result: IntersectionList[] = [];
  friends.reduce((resultSoFar, friend) => {
    const colleague = colleagues.find(col => col.name === friend.name);
    if (colleague) {
      const combined: IntersectionList = {name: friend.name,age: friend.age,contact: { email: colleague.contact.email, extension: colleague.contact.extension } };
      resultSoFar.push(combined);
    }
    return resultSoFar;
  }, result);
  return result;
